  ...
  return (
    <Container className="table">
      <Row className="table-header">
        <Col xs={1}>#</Col>
        <Col sm={3} xs={2}>Date</Col>
        <Col sm={2} xs={3}>Status</Col>
        <Col xs={2}>Temp. High(°F)</Col>
        <Col xs={2}>Temp. Low(°F)</Col>
        {/* If it is xs-sized-device, show "Wind Speed" without (mph) */}
        <Col sm={2} className="d-none d-sm-block">Wind Speed(mph)</Col>
        <Col xs={2} className="d-block d-sm-none">Wind Speed</Col>
      </Row>
      {weeklyWeather.map((day: IWeatherDetails, index: number) => (
        <Row className="table-row" key={index}>
          <Col xs={1}>{index + 1}</Col>
          <Col sm={3} xs={2} className="link">{convertDate(day.startTime)}</Col>
          <Col sm={2} xs={3}>
            <img src={weatherCodes[day.values.weatherCode].icon} alt="icon" style={{
              width: "50px",
              height: "50px",
              margin: "-10px -5px"
            }}/>
...
export default function DayView() {
  ...
  return (
    (weeklyWeather && <DayViewTable weeklyWeather={weeklyWeather} />) || (
      <Alert variant="warning" className="mt-3">No day view data available</Alert>
    )
  );
}
